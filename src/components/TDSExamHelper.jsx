...
export default function TDSExamHelper() {
    ...
    new Promise((resolve) => {
      ...
      reader.onload = () => {
        if (type.includes("text") || type.includes("csv") || type.includes("json")) {
          ...
          resolve({ name: file.name, type, content: `[File ${file.name} (${type}) omitted for brevity]` });
        }
      };
  ...
  const handleSubmit = async () => {
    ...
    if (!question.trim() && files.length === 0) {
      setError("Please enter a question or upload files."); return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
          <h1 className="text-3xl font-bold">TDS Exam Helper (Claude)</h1>
          <p className="text-blue-100 text-sm">Claude-powered verified TDS assistant.</p>
        </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <textarea value={question} onChange={(e) => setQuestion(e.target.value)}
              placeholder="e.g. Write a Python script to query a CSV"
          ...
          <div className="flex items-center space-x-3">
            <button onClick={handleSubmit} disabled={loading}
              ...
              {loading ? <span className="flex items-center"><Loader className="animate-spin mr-2" size={16}/>Analyzing...</span> : "Run Task"}
            </button>
